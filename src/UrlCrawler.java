...
public class UrlCrawler {
	 ...
	 * for correct number of arguments if (args.length < 4) { System.out
	 ...
	 * e.getMessage()); System.exit(-1); } indexDirectory = indexDir.toString();
	 */
	/*
	 * String seedURL =
	 * "http://www.dke-research.de/findke/en/Studies/Courses/WS+2017_2018-p-1064.html"
	 * ; String seedURL = "http://www.mkyong.com/"; maxLevel = 2; indexDirectory
	 * = "C:/Users/admin/Desktop/WS_17_18/Information_Retrieval/P02";
	 */

	// }

	public List<PrintObject> run(String seed, String indexDir, int maxDepth)
			throws IOException {
		maxLevel = maxDepth;
			...
			throws IOException 
	{
		...
		if (duplicate == false && depth <= maxLevel) {
			try {
				...
				if (depth < maxLevel) {
					// try to connect with the current (seed-) URL and retrieve
					// HTML
					// document
					...
					depth_++;
					// recursively add all links to the URLlist if it is not in
					// the
					// list
					// already
	...
	private static String normalize(String URL) {
		...
		// divide string and only keep the first part, thus avoiding the
		// anker-part
		{
